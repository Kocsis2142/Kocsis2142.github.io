...
document.querySelector("#easymode").addEventListener ('click', function start () {
    if(runtime === false){
    initGame(1, 30);
    }
});

document.querySelector("#hardmode").addEventListener ('click', function start () {
    if(runtime === false){
    initGame(1, 0);
    }
});
...
function turnC(id) {
    ...
    a.addEventListener('click', function turnCard(){
        if (clickCounter === 1 && a.src.includes("back.png")){
            ...
            a.src = shuffleCards[id].frontimg;},250)
            clicked = a;
...
function countdown( minutes, seconds ) {
    ...
    function updateTimer() {
        ...
        if(runtime == false){
            ...
            timer.innerHTML = (hours ? hours + ':' + twoDigits( mins ) : mins) + ':' + twoDigits( time.getUTCSeconds() );
